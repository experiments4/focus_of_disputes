...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, hidden_state, task_idx):
        return self.down_proj(self.act_fn(self.gate_proj(hidden_state)) * self.up_proj(hidden_state, task_idx), task_idx)
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states, task_idx)
        hidden_states = residual + hidden_states
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        # log.info(f"task_idx before outputs: {task_idx}")
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        model_inputs.update(
            {
                ...
                "use_cache": use_cache,
                "attention_mask": attention_mask,
                "task_idx": torch.tensor(kwargs["task_idx"])
            }
        )
        return model_inputs
