...
class Trainer:
    ...
    def calculate_param_importance(self, model, flag):
        for name, param in model.named_parameters():
            if param.grad is not None:
                # Calculate importance for each column
                importance = torch.abs(param * param.grad) 
                column_importance = importance.mean(dim=0)
                min_val = column_importance.min()
                max_val = column_importance.max()
                if max_val > min_val:
                    normalized_importance = (column_importance - min_val) / (max_val - min_val)
                else:
                    normalized_importance = torch.zeros_like(column_importance)

                update_vector = param.grad
                column_update = update_vector.mean(dim=0)
    
                # Store importance and update vectors
                attr_name = f'{flag}_column_importance'
                if not hasattr(param, f'{flag}_column_importance'):
                    param.attr_name = normalized_importance
                attr_name = f'{flag}_column_update'
                if not hasattr(param, f'{flag}_column_update'):
                    param.attr_name = column_update
                
    def trim_grad(self, model):
        for name, param in model.named_parameters():
            if param.grad is not None and hasattr(param, 'new_column_update') and hasattr(param, 'old_column_update'):
                new_update = torch.norm(param.new_column_update)
                old_update = torch.norm(param.old_column_update)
                
                common = new_update + old_update
        

                norm = torch.norm(common)
                if norm > 0:
                    normalized_common = common / norm
                else:
                    normalized_common = torch.zeros_like(common)
                
                old_common = old_update * normalized_common
                old_pure = old_update - old_common

                if hasattr(param, 'new_column_importance') and hasattr(param, 'old_column_importance'):
                    final_grad = (new_update * param.new_column_importance - 
                                old_pure * param.old_column_importance)
                    
                    # Update parameter gradient
                    if param.grad.shape[1] == final_grad.shape[0]:  # Check dimensions match
                        param.grad = param.grad * final_grad[None, :]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.use_apex:
            ...
            self.accelerator.backward(loss[0], **kwargs)
            self.accelerator.backward(loss[1], **kwargs)
            self.calculate_param_importance(model, 'new')
            self.calculate_param_importance(model, 'old')
            self.trim_grad(model)
